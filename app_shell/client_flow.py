...
<div style="background:#f0f4ff;border:1px solid #bfdbfe;border-radius:10px;
     padding:12px 16px;direction:rtl;text-align:right;margin-bottom:10px">
  <div style="font-size:12px;font-weight:800;color:#1e40af;margin-bottom:3px">
    📤 ייבוא דוח מסלקה פנסיונית
  </div>
  <div style="font-size:11px;color:#3730a3">
    העלה קובץ Excel (XLSX) מגמל-נט או ממשרד האוצר
  </div>
</div>""", unsafe_allow_html=True)
        ...
        with st.spinner("מנתח דוח מסלקה..."):
            try:
                ...
                if err_msg and not result:
                    ...
                    if holdings_raw:
                        # Update case — no page change, stays in wizard
                        ...
                        CaseStore.save(_upd)
                        st.success(f"✅ נטענו {len(holdings_raw)} מוצרים — ממשיכים בתהליך הלקוח.")
                        st.rerun()
                    else:
    ...
    # ── הוספה ידנית של מוצר ────────────────────────────────────────────────
    st.markdown("#### ➕ הוסף מוצר לתיק")
    try:
        from portfolio_analysis.ui import _render_add_form
        from portfolio_analysis.models import get_holdings as _get_h, STATE_KEY as _SK
        # Pass the current merged holdings from case to the form
        _form_holdings = list(case.all_holdings)
        _render_add_form(_form_holdings, df_long)
        # After form may have written to pf_holdings (STATE_KEY), sync back to CaseStore
        import streamlit as _st2
        _pf_h = _st2.session_state.get(_SK) or []
        if _pf_h and len(_pf_h) > len(case.all_holdings):
            # New product was added — sync to case
            _c2 = CaseStore.get()
